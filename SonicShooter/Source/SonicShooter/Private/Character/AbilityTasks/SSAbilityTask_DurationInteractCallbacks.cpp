...
USSAbilityTask_DurationInteractCallbacks::USSAbilityTask_DurationInteractCallbacks(const FObjectInitializer& ObjectInitializer)
...
USSAbilityTask_DurationInteractCallbacks* USSAbilityTask_DurationInteractCallbacks::DurationInteractCallbacks(UGameplayAbility* OwningAbility, ASSCharacter_Shooter* ShooterCharacter, ISSInteractableInterface*& InInteract)
{
	...
	USSAbilityTask_DurationInteractCallbacks* MyObj = NewAbilityTask<USSAbilityTask_DurationInteractCallbacks>(OwningAbility);
...
void USSAbilityTask_DurationInteractCallbacks::Activate()
{
	currentTime = 0;
	continueTimestamp = 0;
	ENetRole role = ShooterCharacter->GetLocalRole();
	if (Interactable->GetDetectType() == ESSDetectType::DETECTTYPE_Overlapped)
	{
		OnPawnLeftOverlapInteractableDelegateHandle = ShooterCharacter->GetInteractorComponent()->OnElementRemovedFromFrameOverlapInteractablesStack.AddUObject(this, &USSAbilityTask_DurationInteractCallbacks::OnPawnLeftOverlapInteractable);
	}
	
}

void USSAbilityTask_DurationInteractCallbacks::TickTask(float DeltaTime)
{
	...
	{
		if (Interactable->GetDetectType() == ESSDetectType::DETECTTYPE_Sweeped)		// If the character's Interaction sweep doesn't detect the same Interactable we started interacting with
		{
...
void USSAbilityTask_DurationInteractCallbacks::OnPawnLeftOverlapInteractable(ISSInteractableInterface*& InteractableThePawnLeft)
{
	ENetRole role = ShooterCharacter->GetLocalRole();
	...
	//		if (ShooterCharacter->CurrentPrioritizedInteractable->GetDetectType() == ESSDetectType::DETECTTYPE_Overlapped)
	//		{
...
void USSAbilityTask_DurationInteractCallbacks::RemoveAllDelegates()
{
...
FString USSAbilityTask_DurationInteractCallbacks::GetDebugString() const
{
	return FString::Printf(TEXT("DurationInteractCallbacks"));
}

void USSAbilityTask_DurationInteractCallbacks::OnDestroy(bool AbilityEnded)
{
	ENetRole d = ShooterCharacter->GetLocalRole();
