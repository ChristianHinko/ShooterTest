...
AFPSTemplateFirearm* UFPSTemplate_CharacterComponent::AddFirearm(TSubclassOf<AFPSTemplateFirearm> FirearmClass, bool SetAsCurrent, bool SetHidden)
{
...
void UFPSTemplate_CharacterComponent::AddExistingFirearm(AFPSTemplateFirearm* Firearm)
{
...
AFPSTemplateFirearm* UFPSTemplate_CharacterComponent::SwitchToFirearm(AFPSTemplateFirearm* Firearm, const FName CurrentFirearmAttachSocket, bool HideCurrentFirearm)
{
	if (IsValid(Firearm))
	{
		...
		if (!HasAuthority())
		{
			Server_SwitchFirearm(Firearm, CurrentFirearmAttachSocket, HideCurrentFirearm);
		}
		if (IsValid(TempFirearm) && CurrentFirearmAttachSocket != NAME_None)
		{
			TempFirearm->AttachToSocket(CurrentFirearmAttachSocket);
		}
		return TempFirearm;
	}
