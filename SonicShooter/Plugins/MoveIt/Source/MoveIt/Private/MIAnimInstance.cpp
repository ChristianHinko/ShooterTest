...
void UMIAnimInstance::NativeUpdateAnimation(float DeltaTime)
{
	...
	AnimSpeed = UKismetMathLibrary::NormalizeToRange(RawSpeed, 0.f, Movement->GetMaxSpeed());
	GaitSpeed = (GaitInterpRate > 0.f) ? FMath::FInterpConstantTo(GaitSpeed, AnimSpeed, DeltaTime, GaitInterpRate) : AnimSpeed;
...
void UMIAnimInstance::ComputeAimOffsets(float DeltaTime)
{
	...
	{
		AimOffsetYaw = FMath::FInterpConstantTo(AimOffsetYaw, -RootYawOffset, DeltaTime, AimOffsetYawRate);
	}
	else
	{
		AimOffsetYaw = -RootYawOffset;
	}
...
{
	return Movement && Movement->IsSprintingAtSpeed();
}
