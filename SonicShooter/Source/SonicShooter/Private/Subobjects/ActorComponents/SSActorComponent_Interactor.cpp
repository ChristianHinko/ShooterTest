...
USSActorComponent_Interactor::USSActorComponent_Interactor()
{
...
void USSActorComponent_Interactor::InitializeComponent()
{
	Super::InitializeComponent();

	OwningShooterCharacter = Cast<ASSCharacter_Shooter>(GetOwner());
	OwningShooterCharacter->GetCapsuleComponent()->OnComponentBeginOverlap.AddDynamic(this, &USSActorComponent_Interactor::OnComponentBeginOverlapCharacterCapsule);
	OwningShooterCharacter->GetCapsuleComponent()->OnComponentEndOverlap.AddDynamic(this, &USSActorComponent_Interactor::OnComponentEndOverlapCharacterCapsule);
}

void USSActorComponent_Interactor::TickComponent(float DeltaTime, ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
...
void USSActorComponent_Interactor::OnComponentBeginOverlapCharacterCapsule(UPrimitiveComponent* OverlappedComp, AActor* OtherActor, UPrimitiveComponent* OtherComp, int32 OtherBodyIndex, bool bFromSweep, const FHitResult& SweepResult)
{
	if (ISSInteractableInterface* Interactable = Cast<ISSInteractableInterface>(OtherActor))
	{
		...
		Interactable->InjectDetectType(ESSDetectType::DETECTTYPE_Overlapped);
		CurrentOverlapInteractablesStack.Push(Interactable);
	}
}
void USSActorComponent_Interactor::OnComponentEndOverlapCharacterCapsule(UPrimitiveComponent* OverlappedComp, AActor* OtherActor, UPrimitiveComponent* OtherComp, int32 OtherBodyIndex)
{
	if (ISSInteractableInterface* Interactable = Cast<ISSInteractableInterface>(OtherActor))
	{
		if (CurrentOverlapInteractablesStack.Num() > 0)
		{
			...
			Interactable->InjectDetectType(ESSDetectType::DETECTTYPE_NotDetected);		// Give a value of not detected just in case
		}
	}
}
...
ISSInteractableInterface* USSActorComponent_Interactor::ScanForCurrentPrioritizedInteractable(FHitResult& OutHit)
{
	...
	if (GetWorld() && OwningShooterCharacter->GetFollowCamera())
	{
		...
		if (bBlockingHit)
		{
			if (ISSInteractableInterface* BlockingHitInteractable = Cast<ISSInteractableInterface>(OutHit.GetActor()))
			{
				if (BlockingHitInteractable->GetCanCurrentlyBeInteractedWith())
				{
					BlockingHitInteractable->InjectDetectType(ESSDetectType::DETECTTYPE_Sweeped);
...
void USSActorComponent_Interactor::TryInteract()
{
	if (CurrentPrioritizedInteractable)
	{
