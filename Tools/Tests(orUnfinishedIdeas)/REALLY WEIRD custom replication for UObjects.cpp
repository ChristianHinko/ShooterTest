...
//void UAS_Character::PreReplication(IRepChangedPropertyTracker& ChangedPropertyTracker)
//{
...
void UAS_Character::Tick(float DeltaTime)
{
	if (bStaminaDraining)
	{
		...
		if (GetStamina() > 0) // we are draining:
		{
			SetStamina(GetStamina() - (GetStaminaDrain() * DeltaTime));
		}
		else // we are fully drained:
		{
			SetStamina(0);
	...
	else
	{
		...
		if (GetStamina() < GetMaxStamina()) // we are regening:
		{
	...
	//UKismetSystemLibrary::PrintString(this, "stam: " + FString::SanitizeFloat(GetStamina()), true, false);
}
void UAS_Character::SetShouldTick(bool newShouldTick)
{
	if (bShouldTick != newShouldTick)
	{
		...
		//FRepChangedPropertyTracker& ChangedPropertyTracker = *(NetDriver->FindOrCreateRepChangedPropertyTracker(this).Get());
		...
		if (!GlobalPropertyTrackerPtr)
		{
			...
			//NetDriver->GetObjectClassRepLayout(this->GetClass())->InitChangedTracker(Tracker);


			TSharedPtr<FRepLayout>* RepLayoutPtr = NetDriver->RepLayoutMap.Find(this->GetClass());

			if (!RepLayoutPtr)
			{
				ECreateRepLayoutFlags Flags = NetDriver->MaySendProperties() ? ECreateRepLayoutFlags::MaySendProperties : ECreateRepLayoutFlags::None;
				RepLayoutPtr = &(NetDriver->RepLayoutMap).Add(this->GetClass(), FRepLayout::CreateFromClass(this->GetClass(), NetDriver->ServerConnection, Flags));
			}

			(*RepLayoutPtr)->InitChangedTracker(Tracker);


			GlobalPropertyTrackerPtr = &(NetDriver->RepChangedPropertyTrackerMap).Add(this, UNetDriver::FRepChangedPropertyTrackerWrapper(this, TSharedPtr<FRepChangedPropertyTracker>(Tracker)));
		}
		FRepChangedPropertyTracker ChangedPropertyTracker = *(GlobalPropertyTrackerPtr->RepChangedPropertyTracker.Get());
