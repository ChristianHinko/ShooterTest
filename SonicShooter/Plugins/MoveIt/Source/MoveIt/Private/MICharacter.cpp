...
bool AMICharacter::IsViewComponentStateActive_Implementation(uint8 State) const
{
	...
	case 1:  // Sprint
		return bIsSprinting && !bIsCrouchRunning;
	case 2:  // Floor Slide
...
{
	// Character is not being killed, movement is active, we are on the ground rather than in the air, and not simulating physics
	return !bIsSprinting && MICharacterMovement && MICharacterMovement->IsActive() && !IsPendingKillPending() && MICharacterMovement->IsMovingOnGround() && GetRootComponent() && !GetRootComponent()->IsSimulatingPhysics();
}
...
void AMICharacter::OnCollideWith(UPrimitiveComponent* HitComponent, AActor* OtherActor, UPrimitiveComponent* OtherComp, FVector NormalImpulse, const FHitResult& Hit)
{
	...
	const bool bIsPlayingRootMotion = MICharacterMovement->HasAnimRootMotion();
...
void AMICharacter::HandleImpactCharacter(AMICharacter* OtherCharacter, const FVector& ImpactNormal, const FVector& ImpactVelocity, bool bInstigator)
{
	...
	if (!bHitCharacterEnabled || bMontageBlockHit || bRootMotionBlockHit)
	{
		return;
	}
