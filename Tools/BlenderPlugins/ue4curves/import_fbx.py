...
if "bpy" in locals():
    import importlib
    if "bbparse_fbx" in locals():
        importlib.reload(bbparse_fbx)
    if "bbfbx_utils" in locals():
        importlib.reload(bbfbx_utils)
...
# -----
# Utils
from . import bbparse_fbx, bbfbx_utils

from .bbparse_fbx import (
    bbdata_types,
    FBXElem,
)
from .bbfbx_utils import (
    PerfMon,
...
MAT_CONVERT_BONE = bbfbx_utils.MAT_CONVERT_BONE.inverted()
MAT_CONVERT_LIGHT = bbfbx_utils.MAT_CONVERT_LIGHT.inverted()
MAT_CONVERT_CAMERA = bbfbx_utils.MAT_CONVERT_CAMERA.inverted()
...
def elem_find_first_string(elem, id_search):
    ...
    if fbx_item is not None and fbx_item.props:  # Do not error on complete empty properties (see T45291).
        ...
        assert(fbx_item.props_type[0] == bbdata_types.STRING)
...
def elem_find_first_bytes(elem, id_search, decode=True):
    ...
    if fbx_item is not None and fbx_item.props:  # Do not error on complete empty properties (see T45291).
        assert(len(fbx_item.props) == 1)
        assert(fbx_item.props_type[0] == bbdata_types.BYTES)
...
def elem_split_name_class(elem):
    assert(elem.props_type[-2] == bbdata_types.STRING)
...
def elem_split_name_class_nodeattr(elem):
    ...
    assert(elem.props_type[-1] == bbdata_types.STRING)
    elem_class = elem.props[-1]
    return elem_name, elem_class


def elem_uuid(elem):
    assert(elem.props_type[0] == bbdata_types.INT64)
    return elem.props[0]
...
def elem_props_get_color_rgb(elem, elem_prop_id, default=None):
    ...
    if elem_prop is not None:
        ...
        assert(elem_prop.props_type[4:7] == bytes((bbdata_types.FLOAT64,)) * 3)
...
def elem_props_get_number(elem, elem_prop_id, default=None):
    ...
    if elem_prop is not None:
        ...
        assert(elem_prop.props_type[4] == bbdata_types.FLOAT64)
...
def elem_props_get_integer(elem, elem_prop_id, default=None):
    ...
    if elem_prop is not None:
        ...
        # we could allow other number types
        assert(elem_prop.props_type[4] in {bbdata_types.INT32, bbdata_types.INT64})
...
def elem_props_get_bool(elem, elem_prop_id, default=None):
    ...
    if elem_prop is not None:
        ...
        # we could allow other number types
        assert(elem_prop.props_type[4] == bbdata_types.INT32)
        ...
        for fbx_prop in fbx_obj_props.elems:
            ...
            if b'U' in fbx_prop.props[3]:
                if fbx_prop.props[0] == b'UDP3DSMAX':
                    ...
                    assert(fbx_prop.props_type[4] == bbdata_types.STRING)
                    ...
                    if prop_type in {b'Vector', b'Vector3D', b'Color', b'ColorRGB'}:
                        assert(fbx_prop.props_type[4:7] == bytes((bbdata_types.FLOAT64,)) * 3)
                        blen_obj[prop_name] = fbx_prop.props[4:7]
                    elif prop_type in {b'Vector4', b'ColorRGBA'}:
                        assert(fbx_prop.props_type[4:8] == bytes((bbdata_types.FLOAT64,)) * 4)
                        blen_obj[prop_name] = fbx_prop.props[4:8]
                    elif prop_type == b'Vector2D':
                        assert(fbx_prop.props_type[4:6] == bytes((bbdata_types.FLOAT64,)) * 2)
                        blen_obj[prop_name] = fbx_prop.props[4:6]
                    elif prop_type in {b'Integer', b'int'}:
                        assert(fbx_prop.props_type[4] == bbdata_types.INT32)
                        ...
                        assert(fbx_prop.props_type[4] == bbdata_types.FLOAT64)
                        blen_obj[prop_name] = fbx_prop.props[4]
                    elif prop_type in {b'Float', b'float'}:
                        assert(fbx_prop.props_type[4] == bbdata_types.FLOAT32)
                        blen_obj[prop_name] = fbx_prop.props[4]
                    elif prop_type in {b'Bool', b'bool'}:
                        assert(fbx_prop.props_type[4] == bbdata_types.INT32)
                        blen_obj[prop_name] = fbx_prop.props[4] != 0
                    elif prop_type in {b'Enum', b'enum'}:
                        assert(fbx_prop.props_type[4:6] == bytes((bbdata_types.INT32, bbdata_types.STRING)))
...
def blen_read_animations_curves_iter(fbx_curves, blen_start_offset, fbx_start_offset, fps):
    ...
    # of FBX curves later.
    from .bbfbx_utils import FBX_KTIME
...
def blen_read_animations_action_item(action, item, cnodes, fps, anim_offset):
    ...
    """
    from bpy.types import Object, PoseBone, ShapeKey, Material, Camera, Property
    ...
    if isinstance(item, Material):
        ...
        for frame, values in blen_read_animations_curves_iter(fbx_curves, anim_offset, 0, fps):
            for v, (fbxprop, channel, _fbx_acdata) in values:
                if channel==4:
                    varname = fbxprop.decode('utf-8')
                    obj = bpy.data.objects.get('root')
                    obj[varname] = v
                    obj.keyframe_insert(data_path='["'+varname+'"]', frame=frame)
...
def load(operator, context, filepath="",
    ...
    from . import bbparse_fbx
    from .bbfbx_utils import RIGHT_HAND_AXES, FBX_FRAMERATES
    ...
    try:
        elem_root, version = bbparse_fbx.parse(filepath)
        ...
        def _():
            ...
            for acn_uuid, fbx_acnitem in fbx_table_nodes.items():
                ...
                for n_uuid, n_ctype in fbx_connection_map.get(acn_uuid, ()):
                    ...
                    if lnk_prop in {b'Lcl Translation', b'Lcl Rotation', b'Lcl Scaling'}:
                        ...
                        #Custom Variable (uses root)
                        ob = fbx_helper_nodes.get(n_uuid, None)
                        fbx_item = fbx_table_nodes.get(n_ctype.props[1], n_uuid)
                        #fbx_item = fbx_item[0].props[1]
                        print("A Custom Variable was found:", lnk_prop, "id:", n_ctype.props[1])
                        items.append((ob, lnk_prop))
            ...
            for ac_uuid, fbx_acitem in fbx_table_nodes.items():
                ...
                for acn_uuid, acn_ctype in fbx_connection_map.get(ac_uuid, ()):
                    ...
                    if channel is None:
                        #Custom Curve 2/26/2019
                        channel = 4 #overwrites all the variables to be custom channel 4
