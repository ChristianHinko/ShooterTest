...
void UMIViewComponent::BeginPlay()
{
	...
	for (UCameraComponent* const Camera : UpdatedCameras)
	{
		...
		if (SpringArm)
		{
			UpdatedCameraRelativeLocations.Add(Camera, SpringArm->GetRelativeTransform().GetLocation());
		}
		else
		{
			UpdatedCameraRelativeLocations.Add(Camera, Camera->GetRelativeTransform().GetLocation());
		}
	}
	...
	if (!StateBlend.IsComplete())
	{
		...
		for (UCameraComponent* Camera : UpdatedCameras)
		{
			if (Camera)
			{
				...
				USceneComponent* UpdatedComponent = Camera;

				// Must move the spring arm instead of the camera if present, or it will not account for collision
				if (Camera->GetAttachParent() && Camera->GetAttachParent()->IsA<USpringArmComponent>())
				{
					UpdatedComponent = Camera->GetAttachParent();
				}
				
				UpdatedComponent->SetRelativeLocation(UpdatedCameraRelativeLocations[Camera] + StateViewOffset + UpdatedComponent->GetComponentRotation().UnrotateVector(HHeight));
			}
		}
	}
}
