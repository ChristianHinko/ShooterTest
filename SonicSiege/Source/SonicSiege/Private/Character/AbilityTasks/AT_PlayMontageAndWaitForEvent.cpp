...
#include "Character/AbilityTasks/AT_PlayMontageAndWaitForEvent.h"
...
USSAbilitySystemComponent* UAT_PlayMontageAndWaitForEvent::GetTargetASC()
{
...
void UAT_PlayMontageAndWaitForEvent::Activate()
{
	...
	USSAbilitySystemComponent* GDAbilitySystemComponent = GetTargetASC();
	...
	if (GDAbilitySystemComponent)
	{
		...
		{
			UE_LOG(LogTemp, Warning, TEXT("%s() call to PlayMontage failed!"), *FString(__FUNCTION__));
		}
	}
	else
	{
	...
	if (!bPlayedMontage)
	{
		UE_LOG(LogTemp, Warning, TEXT("%s() called in Ability %s failed to play montage %s; Task Instance Name %s."), *FString(__FUNCTION__), *Ability->GetName(), *GetNameSafe(MontageToPlay), *InstanceName.ToString());
		if (ShouldBroadcastAbilityTaskDelegates())
		{
