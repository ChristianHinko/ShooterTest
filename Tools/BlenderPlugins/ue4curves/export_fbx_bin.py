...
if "bpy" in locals():
    import importlib
    if "bbencode_bin" in locals():
        importlib.reload(bbencode_bin)
    if "bbdata_types" in locals():
        importlib.reload(bbdata_types)
...
from . import bbencode_bin, bbdata_types, bbfbx_utils
...
def fbx_data_mesh_elements(root, me_obj, scene_data, done_meshes):
    ...
    # Vertex cos.
    t_co = array.array(bbdata_types.ARRAY_FLOAT64, (0.0,)) * len(me.vertices) * 3
    ...
    t_pvi = array.array(bbdata_types.ARRAY_INT32, (0,)) * loop_nbr
    ...
    #       (like e.g. crease).
    t_eli = array.array(bbdata_types.ARRAY_INT32)
    edges_map = {}
    edges_nbr = 0
    if t_ls and t_pvi:
        t_ls = set(t_ls)
    ...
    if smooth_type in {'FACE', 'EDGE'}:
        ...
        _map = b""
        if smooth_type == 'FACE':
            t_ps = array.array(bbdata_types.ARRAY_INT32, (0,)) * len(me.polygons)
            me.polygons.foreach_get("use_smooth", t_ps)
            _map = b"ByPolygon"
        else:  # EDGE
            # Write Edge Smoothing.
            # Note edge is sharp also if it's used by more than two faces, or one of its faces is flat.
            t_ps = array.array(bbdata_types.ARRAY_INT32, (0,)) * edges_nbr
            sharp_edges = set()
            temp_sharp_edges = {}
    ...
    if write_normals:
        ...
        t_ln = array.array(bbdata_types.ARRAY_FLOAT64, (0.0,)) * len(me.loops) * 3
        ...
        if 0:
            ...
            # t_lnw = array.array(bbdata_types.ARRAY_FLOAT64, (0.0,)) * len(ln2idx)
            ...
            # Normal weights, no idea what it is.
            # t_ln = array.array(bbdata_types.ARRAY_FLOAT64, (0.0,)) * len(me.loops)
        ...
        if scene_data.settings.use_tspace:
            ...
            if tspacenumber:
                t_ln = array.array(bbdata_types.ARRAY_FLOAT64, (0.0,)) * len(me.loops) * 3
                # t_lnw = array.array(bbdata_types.ARRAY_FLOAT64, (0.0,)) * len(me.loops)
    ...
    if vcolnumber:
        ...
        t_lc = array.array(bbdata_types.ARRAY_FLOAT64, (0.0,)) * len(me.loops) * 4
    ...
    if uvnumber:
        ...
        t_luv = array.array(bbdata_types.ARRAY_FLOAT64, (0.0,)) * len(me.loops) * 2
        t_lvidx = array.array(bbdata_types.ARRAY_INT32, (0,)) * len(me.loops)
    ...
    if me_fbxmaterials_idx is not None:
        ...
        if me_fbxmaterials_idx and me_blmaterials:
            ...
            if nbr_mats > 1:
                t_pm = array.array(bbdata_types.ARRAY_INT32, (0,)) * len(me.polygons)
                ...
                t_pm = array.array(bbdata_types.ARRAY_INT32, _gen)
...
def fbx_data_from_scene(scene, depsgraph, settings):
    ...
    for me_key, me, _free in data_meshes.values():
        ...
        _cos = array.array(bbdata_types.ARRAY_FLOAT64, (0.0,)) * len(me.vertices) * 3
...
def save_single(operator, scene, depsgraph, filepath="",
    ...
    bbencode_bin.write(filepath, root, FBX_VERSION)
