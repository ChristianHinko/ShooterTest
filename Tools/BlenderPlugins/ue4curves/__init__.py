######################################################################################################
#                    				UE4 FBX Import/Export                                            #
######################################################################################################
#                                                                   #                                #
# Export UE4 ready FBX with that include custom animation curves.   #                                #
#                                                                   #          Created By            #
#  This software is provided as is, with the following restriction. #        Andy @ MostHostLA       #
#   You may NOT include this software into a PAID application or    #                                #
#     make a profit by directly selling portions of this code.      #       www.mosthostla.com       #
#                                                                   #                                #
#                                                                   #                                #
#*******************************************************************#********************************#
#                                                                                                    #
#                  A very modifued version of the standard FBX Import/Export.                        #
#                Original code created by Campbell Barton and Bastien Montagne.                      #
#   Thanks to their previous work and the choice of GPL license it was possible to re-work the code  #
#              to birng a specific FBX importer for UE4 that attempts to allow for all the           #
#                    different export options needed within a professional enviroment.               #
#                      Particularly Animation Curves; likely why you got this add-on                 #
#                                                                                 (2K screens rulke) #
######################################################################################################
bl_info = {
	"name": "ue4curves",
	"description": "Export animation curves and better UE4 ready FBX files.",
	"author": "Andy Fattoracci @ MostHostLA.com",
	"version": (1, 0, 0),
	"blender": (2, 80, 0),
	"location": "Armature properties, Bone properties, View3d tools panel, Armature Add menu",
	"warning": "", # used for warning icon and text in addons panel
	"tracker_url": "http://mosthostla.com/gamedev/contact/",
	"wiki_url": "http://mosthostla.com/gamedev/ue4curves/",
	"category": "Export"}
...
# ------------------------------------------------------------------------
#    FBX Import/Export Actions - lifted from fbx_scene_io
# ------------------------------------------------------------------------
@orientation_helper(axis_forward='-Z', axis_up='Y')
class UE4FBX_OT_importfbx(bpy.types.Operator, ImportHelper):
	"""Load a FBX file UE4 version"""
	bl_idname = "ue4fbx.importfbx"
	bl_label = "UE4 Import FBX"
...
@orientation_helper(axis_forward='-Z', axis_up='Y')
class UE4FBX_OT_exportfbx(bpy.types.Operator, ExportHelper):
	"""Write a FBX file"""
	bl_idname = "ue4fbx.exportfbx"
	bl_label = "UE4 Export FBX"
	bl_options = {'UNDO', 'PRESET'}
	...
	def draw(self, context):
		...
		if self.ui_tab == 'MAIN':
			...
			sub.prop(self, "apply_unit_scale", text="", icon='FULLSCREEN_ENTER')
...
def menu_import(self, context):
	self.layout.operator(UE4FBX_OT_importfbx.bl_idname, text="UE4 FBX (.fbx)")


def menu_export(self, context):
	self.layout.operator(UE4FBX_OT_exportfbx.bl_idname, text="UE4 FBX (.fbx)")
	
# ------------------------------------------------------------------------
#    Registration
# ------------------------------------------------------------------------
def register():
	bpy.utils.register_class(UE4FBX_OT_importfbx)
	bpy.utils.register_class(UE4FBX_OT_exportfbx)
	bpy.types.TOPBAR_MT_file_import.append(menu_import)
	bpy.types.TOPBAR_MT_file_export.append(menu_export)
def unregister():
	bpy.types.TOPBAR_MT_file_import.remove(menu_import)
	bpy.types.TOPBAR_MT_file_export.remove(menu_export)
	bpy.utils.unregister_class(UE4FBX_OT_exportfbx)
	bpy.utils.unregister_class(UE4FBX_OT_importfbx)
