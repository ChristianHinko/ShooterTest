...
void USSInventoryComponent_Active::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
	...
	DOREPLIFETIME_WITH_PARAMS_FAST(USSInventoryComponent_Active, ActiveItemHistory, Params);
}


USSInventoryComponent_Active::USSInventoryComponent_Active(const FObjectInitializer& ObjectInitializer)
...
void USSInventoryComponent_Active::InitializeComponent()
{
	Super::InitializeComponent();

	OnAttributeSetCreated.AddDynamic(this, &USSInventoryComponent_Active::OnAttributeSetCreatedEvent);
}

void USSInventoryComponent_Active::BeginPlay()
{
...
bool USSInventoryComponent_Active::IsActiveItemSlotIndexValid(int32 InActiveItemSlot)
{
...
void USSInventoryComponent_Active::OnItemEquipped(UArcInventoryComponent* Inventory, const FArcInventoryItemSlotReference& ItemSlotRef, UArcItemStack* ItemStack, UArcItemStack* PreviousItemStack)
{
...
void USSInventoryComponent_Active::OnAttributeSetCreatedEvent(UArcInventoryComponent_Equippable* Inventory, UAttributeSet* AttributeSet, UArcItemStack* AttributeSource)
{
	// Tells us to apply the default stats Effect
	bCreatedAttributeSets = true;
}

void USSInventoryComponent_Active::MakeItemActive(int32 NewActiveItemSlot)
{
	...
	if (bCreatedAttributeSets)
	{
		...
		if (IsValid(ASC))
		{
			USSItemDefinition_Active* SSItemDefinition = Cast<USSItemDefinition_Active>(ActiveItemStack->GetItemDefinition().GetDefaultObject());
			if (IsValid(SSItemDefinition))
			{
				...
				{
					UE_LOG(LogArcInventorySetup, Warning, TEXT("%s() Tried to apply the InitializationEffectTSub but failed. Maybe check if you filled out your USSItemDefinition_Active::InitializationEffectTSub correctly in Blueprint"), ANSI_TO_TCHAR(__FUNCTION__));
				}
			}
		}
...
bool USSInventoryComponent_Active::MakeItemActive_Internal(const FArcInventoryItemSlotReference& ItemSlot, UArcItemStack* ItemStack)
{
...
void USSInventoryComponent_Active::AddToActiveItemHistory(const FArcInventoryItemSlotReference& NewActiveItemSlotReference)
{
	int32 SizeChange = 0;
