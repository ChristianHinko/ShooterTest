...
void UMICharacterMovementComponent::CheckMovementInput(float DeltaTime)
{
	...
	// Sprinting and crouch running
	{
		...
		if (!bWasSprinting && MICharacterOwner->bIsSprinting && !MICharacterOwner->bIsCrouchRunning)
		{
			MICharacterOwner->OnStartSprint();
		}
		// Event sprinting has stopped
		else if (bWasSprinting && !MICharacterOwner->bIsSprinting && !bWasCrouchRunning)
		{
...
void UMICharacterMovementComponent::CalcVelocity(float DeltaTime, float Friction, bool bFluid, float BrakingDeceleration)
{
	...
	else if (IsSprintingAtSpeed() && IsMovingOnGround())
	{
...
void UMICharacterMovementComponent::StartFloorSlide(bool bClientSimulation)
{
	...
	{
		if (!bFloorSlideGroundSpeedBoostOnlyFromSprint || IsSprintingAtSpeed() || IsFalling())
		{
...
{
	return MICharacterOwner != nullptr && MICharacterOwner->bIsSprinting && !IsCrouchRunning();
}

bool UMICharacterMovementComponent::IsSprintingAtSpeed() const
{
	return IsSprinting() && Velocity.Size2D() >= (Super::GetMaxSpeed() * (IsMovingBackwards() ? MoveBackwardsSpeedMultiplier : 1.f));
}
