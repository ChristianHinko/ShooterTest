...
void UInteractorComponent::InitializeComponent()
{
	...
	OwningSiegeCharacter = Cast<ASiegeCharacter>(GetOwner());
	OwningSiegeCharacter->GetCapsuleComponent()->OnComponentBeginOverlap.AddDynamic(this, &UInteractorComponent::OnComponentBeginOverlapCharacterCapsule);
	OwningSiegeCharacter->GetCapsuleComponent()->OnComponentEndOverlap.AddDynamic(this, &UInteractorComponent::OnComponentEndOverlapCharacterCapsule);
}
...
void UInteractorComponent::TickComponent(float DeltaTime, ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
	...
	if (OwningSiegeCharacter->HasAuthority() || OwningSiegeCharacter->IsLocallyControlled())	// Don't run for simulated proxies
	{
		...
		if (CurrentPrioritizedInteractable)
		{
			if (CurrentPrioritizedInteractable != LastPrioritizedInteractable)
			{
				...
				if (CurrentPrioritizedInteractable->bShouldFireDetectionEvents)
					CurrentPrioritizedInteractable->OnInitialDetect(OwningSiegeCharacter);

				if (LastPrioritizedInteractable)
				{
					if (LastPrioritizedInteractable->bShouldFireDetectionEvents)
						LastPrioritizedInteractable->OnEndDetect(OwningSiegeCharacter);
				}
			...
			{
				if (CurrentPrioritizedInteractable->bShouldFireDetectionEvents)
					CurrentPrioritizedInteractable->OnConsecutiveDetect(OwningSiegeCharacter);
			}
...
IInteractable* UInteractorComponent::ScanForCurrentPrioritizedInteractable(FHitResult& OutHit)
{
	...
	if (GetWorld() && OwningSiegeCharacter->GetFollowCamera())
	{
		FVector StartLocation = OwningSiegeCharacter->GetFollowCamera()->GetComponentLocation();
		FVector EndLocation = StartLocation + (OwningSiegeCharacter->GetFollowCamera()->GetForwardVector() * InteractSweepDistance);
...
void UInteractorComponent::TryInteract()
{
	if (CurrentPrioritizedInteractable)
	{
		...
		{
			OwningSiegeCharacter->GetAbilitySystemComponent()->TryActivateAbility(OwningSiegeCharacter->InteractInstantAbilitySpecHandle);
		}
		if (CurrentPrioritizedInteractable->GetIsManualDurationInteract())
		{
			OwningSiegeCharacter->GetAbilitySystemComponent()->TryActivateAbility(OwningSiegeCharacter->InteractDurationAbilitySpecHandle);
		}
