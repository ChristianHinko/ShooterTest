...
#include "Inventory/Item/Gun/SSGameplayEffectExecutionCalculation_GunDealDamage.h"
...
USSGameplayEffectExecutionCalculation_GunDealDamage::USSGameplayEffectExecutionCalculation_GunDealDamage()
{
	RelevantAttributesToCapture.Add(GetDamageStatics().IncomingDamageDef);
}

void USSGameplayEffectExecutionCalculation_GunDealDamage::Execute_Implementation(const FGameplayEffectCustomExecutionParameters& ExecutionParams, OUT FGameplayEffectCustomExecutionOutput& OutExecutionOutput) const
{
	...
	FSSGameplayEffectContext_Shooter* Context = static_cast<FSSGameplayEffectContext_Shooter*>(MutableSpec->GetContext().Get());
