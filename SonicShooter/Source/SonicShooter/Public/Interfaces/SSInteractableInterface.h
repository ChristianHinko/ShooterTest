...
#include "SSInteractableInterface.generated.h"

class ASSCharacter_Shooter;

/** Describes the detection state */
UENUM()
enum class ESSDetectType
{
...
UENUM()
enum class ESSDurationInteractEndReason
{
...
class USSInteractableInterface : public UInterface
{
	GENERATED_BODY()
};

/**
 *	Might seem kind of unclear how to make an actor interactable but that is because the character is what finds interactables and assignes the it an ESSDetectType. You don't set the ESSDetectType, it gets set by the character who detected the interactable.
 ...
 */
class SONICSHOOTER_API ISSInteractableInterface
{
	GENERATED_BODY()

public:
	ISSInteractableInterface();
	...
	void InjectDetectType(ESSDetectType newDetectType);
	/** WARNING: Implementors don't touch! External use only! */
...
#pragma region InstantInteraction
	...
	virtual void OnInstantInteract(ASSCharacter_Shooter* InteractingCharacter);
#pragma endregion
...
#pragma region DurationInteraction
	...
	virtual void OnDurationInteractBegin(ASSCharacter_Shooter* InteractingCharacter);
	// Called every frame during a duration interaction (while interact input is down)
	virtual void InteractingTick(ASSCharacter_Shooter* InteractingCharacter, float DeltaTime, float CurrentInteractionTime);
	// Called anytime a duration interaction ends (whatever the reason may be)
	virtual void OnDurationInteractEnd(ASSCharacter_Shooter* InteractingCharacter, ESSDurationInteractEndReason DurationInteractEndReason, float InteractionTime);
#pragma endregion
...
#pragma region Detection Events
	...
	virtual void OnInitialDetect(ASSCharacter_Shooter* InteractingCharacter);
	// This remains the player's current interactable
	virtual void OnConsecutiveDetect(ASSCharacter_Shooter* InteractingCharacter);
	// This is no longer the player's current interactable
	virtual void OnEndDetect(ASSCharacter_Shooter* InteractingCharacter);
#pragma endregion
