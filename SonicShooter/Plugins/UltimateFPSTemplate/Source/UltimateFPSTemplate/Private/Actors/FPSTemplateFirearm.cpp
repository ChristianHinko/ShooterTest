...
float AFPSTemplateFirearm::GetStockLengthOfPull()
{
	if (StockComponent)
	{
		if (IsValid(StockComponent->GetPart()))
		{
			...
			{
				if (CharacterComponent && CharacterComponent->IsLocallyControlled())
				{
...
void AFPSTemplateFirearm::OnRep_AttachedToSocket()
{
	if (CharacterComponent)
	{
		if (!HasAuthority())
		{
			FPSLog(TEXT("OnRep_AttachedToSocket"));
		}
...
void AFPSTemplateFirearm::AttachToSocket(const FName Socket)
{
	...
	if (!HasAuthority())
	{
		FPSLog(TEXT("AttachToSocket"));
	}
	if (Socket != NAME_None && CharacterComponent)
	{
		if (!HasAuthority())
		{
			FPSLog(TEXT("AttachToSocket: CharacterComponent VALID"));
		}
		//AttachedToSocket = Socket;
